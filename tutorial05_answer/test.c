...
static void test_parse_object() {
    ...
    EXPECT_EQ_STRING("n", lept_get_object_key(&v, 0), lept_get_object_key_size(&v, 0));
    EXPECT_EQ_INT(LEPT_NULL,   lept_get_type(lept_get_object_value(&v, 0)));
    EXPECT_EQ_STRING("f", lept_get_object_key(&v, 1), lept_get_object_key_size(&v, 1));
    EXPECT_EQ_INT(LEPT_FALSE,  lept_get_type(lept_get_object_value(&v, 1)));
    EXPECT_EQ_STRING("t", lept_get_object_key(&v, 2), lept_get_object_key_size(&v, 2));
    EXPECT_EQ_INT(LEPT_TRUE,   lept_get_type(lept_get_object_value(&v, 2)));
    EXPECT_EQ_STRING("i", lept_get_object_key(&v, 3), lept_get_object_key_size(&v, 3));
    ...
    EXPECT_EQ_STRING("s", lept_get_object_key(&v, 4), lept_get_object_key_size(&v, 4));
    ...
    EXPECT_EQ_STRING("a", lept_get_object_key(&v, 5), lept_get_object_key_size(&v, 5));
    ...
    EXPECT_EQ_STRING("o", lept_get_object_key(&v, 6), lept_get_object_key_size(&v, 6));
    {
        ...
        for (i = 0; i < 3; i++) {
            ...
            EXPECT_EQ_SIZE_T(1, lept_get_object_key_size(o, i));
...
static void test_parse_miss_key() {
    ...
    TEST_ERROR(LEPT_PARSE_MISS_KEY, "{1[");
    TEST_ERROR(LEPT_PARSE_MISS_KEY, "{1}");
    TEST_ERROR(LEPT_PARSE_MISS_KEY, "{  : 2}");   
    TEST_ERROR(LEPT_PARSE_MISS_KEY, "{\"a\" : 7, }");      
}

static void test_miss_colon() {
    TEST_ERROR(LEPT_PARSE_MISS_COLON, "{\"a\" 2}");   
    TEST_ERROR(LEPT_PARSE_MISS_COLON, "{\"a\", 2}");  
    TEST_ERROR(LEPT_PARSE_MISS_COLON, "{\"a\" : 2, \"hello\"}");   
    TEST_ERROR(LEPT_PARSE_MISS_COLON, "{\"a\" : 2, \"hello\"  123 221}");   
}

static void test_parse_miss_comma_or_curly_bracket() {
    TEST_ERROR(LEPT_PARSE_MISS_COMMA_OR_CURLY_BRACKET, "{\"a\" : 2  ");      
    TEST_ERROR(LEPT_PARSE_MISS_COMMA_OR_CURLY_BRACKET, "{\"a\" : 2  \"hello\"");
}
